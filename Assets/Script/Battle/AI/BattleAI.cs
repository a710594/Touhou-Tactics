...
namespace Battle
{
    public class BattleAI
    {
        ...
        protected List<BattleCharacterController> GetTargetList(BattleCharacterControllerData.FactionEnum faction) 
        {
            ...
            {
                if (BattleController.Instance.CharacterList[i].Info.Faction == faction)
                {
            ...
            for (int i = 0; i < _stepList.Count; i++) //我可以移動的範圍
            {
                ...
                for (int j = 0; j < targetList.Count; j++)
                {
                    ...
                    if (targetPosition_v2 != _stepList[i] && rangeList.Contains(targetPosition_v2)) //�ؼЦ�m�P�ڤ��P�B�b�g�{��
                    {
                        ...
                        {
                            Utility.CheckLine(step_v3, targetList[j].transform.position, BattleController.Instance.CharacterList, BattleController.Instance.TileDic, out isBlock, out Vector3 result);
                        }
                        else if (SelectedSkill.Track == TrackEnum.Parabola)
                        {
                            Utility.CheckParabola(step_v3, targetList[j].transform.position, 4, BattleController.Instance.CharacterList, BattleController.Instance.TileDic, out isBlock, out List<Vector3> result); //�n�ɩߪ��u������
                        }
                        else
                        {
        ...
        protected BattleCharacterController GetAttackTarget(List<BattleCharacterController> list)
        {
            ...
            {
                if (_controller.Info.StatusList[i] is ProvocativeStatus)
                {
                    target = ((ProvocativeStatus)_controller.Info.StatusList[i]).Target;
