...
namespace Battle
{
    public class BattleAI
    {
        ...
        protected Dictionary<BattleCharacterInfo, List<Vector2Int>> GetCanHitDic(List<BattleCharacterInfo> targetList) 
        {
            ...
            Dictionary<BattleCharacterInfo, List<Vector2Int>> canHitDic = new Dictionary<BattleCharacterInfo, List<Vector2Int>>(); //<目標,可以攻擊到該目標的地點>
            ...
            for (int i = 0; i < _stepList.Count; i++) //我可以移動的範圍
            {
                ...
                for (int j = 0; j < targetList.Count; j++)
                {
                    ...
                    if (targetPosition_v2 != _stepList[i] && rangeList.Contains(targetPosition_v2)) //目標位置與我不同且在射程內
                    {
                        bool isBlock = false;
                        //檢查射擊是否會被地形阻礙
